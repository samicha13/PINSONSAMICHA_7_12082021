...
exports.updatePost = (req, res) => {
  ...
  models.findOne({ where: { id: req.body.id } }).then((post) => {
    if (post === null) {
    ...
    if (post.idUsers === userId || isAdmin === true) {
      ...
      res.status(401).json({
        message: "vous n'avez pas les droits nécessaires pour modifier le post",
      });
    }
  });
};
...
exports.deletePost = async (req, res) => {
  ...
  const userId = decodedToken.userId;
  const isAdmin = decodedToken.isAdmin;
  const post = await models.findOne({ where: { id: req.body.id } });
  if (post.idUsers === userId || isAdmin === true) {
    comment.destroy({ where: { idPosts: req.body.id } }).then((comment) => {
      console.log(comment);
      post.destroy();
    ...
    res.status(401).json({
      message: "vous n'avez pas les droits nécessaires pour supprimer le post",
    });
  }
};
...
exports.likePost = (req, res, next) => {
  ...
  models
    ...
    .then((post) => {
      ...
      if (usersLikes !== null && usersLikes.indexOf(userId) !== -1) {
        //si userlike pas nul et déjà présent dans le tableau alors
        usersLikes = usersLikes
          .replace(", " + userId, "")
          .replace("" + userId, "");

        return models
          .increment({ likes: -1 }, { where: { id: postId } })
          .then(() => {
            models
              .update({ usersLikes: usersLikes }, { where: { id: postId } })
              .then(() => {
