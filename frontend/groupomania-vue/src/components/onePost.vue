...
<div class="d-flex card mb-3 shadow p-3 mb-5 bg-white rounded">
 ...
 <div v-if="post.idUsers==currentUser.userId || currentUser.isAdmin" class="post-head">
            ...
            <delete-post  :id="post.id"/>
              ...
              <i class="fa fa-edit" @click="editedPost.id=post.id;editedPost.content=post.message"></i>
...
<div class="form-group">
        ...
        />
              ...
              <button class="btn btn-outline-secondary m-4" @click="postUpdate">modifier</button>
            </div>
            <div v-else>
    ...
    <p class="card-text">
       <i class="far fa-user"></i> <small class="text-muted">{{post.User.nom}} {{post.User.prenom}} </small> 
        <br/>  
        <i class="far fa-clock "></i> <small class="text-muted ml-3">{{ moment(post.createdAt).fromNow() }}</small>
        </p>
...
export default {
  ...
  methods: {
    ...
    postUpdate(){
      this.$store
        .dispatch("updatePost",this.editedPost)
       .then(()=>{
